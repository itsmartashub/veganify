...
export default {
  ...
  computed: {
    ...
    //     return newVal;
    //   },
    //   // return this.findMe(this.recipe.id, 0, this.bookmarksIDs.length);
    // },
  },
  ...
  methods: {
    ...
    toggleBookmark() {
      // this.$store.commit("app/SET_NOTIFICATION", { display: true });
      // setTimeout(() => {
      //   this.$store.commit("app/SET_NOTIFICATION", { display: false });
      // }, 3000);
    ...
    findMe(target, start, end) {
      if (start > end) return (this.isBookmarked = false);

      const middle = Math.floor((start + end) / 2);

      if (this.bookmarksIDs[middle] === target) {
        console.log(
          "book middle:",
          this.bookmarksIDs[middle],
          " target: ",
          target
        );
      ...
      if (this.bookmarksIDs[middle] > target) {
        ...
        return this.findMe(target, start, middle - 1);
      }
      ...
      if (this.bookmarksIDs[middle] < target) {
        ...
        return this.findMe(target, middle + 1, end);
      }
    },
  },
